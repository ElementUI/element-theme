...
exports.fonts = function (opts) {
  ...
  var stream = gulp.src(path.resolve(config.themePath, './src/fonts/**'))
    .pipe(opts.minimize ? cssmin({showLog: false}) : nop())
    .pipe(gulp.dest(path.resolve(opts.out || config.output, './fonts')))
    .on('end', function () {
      spin.succeed()
    })
...
exports.build = function (opts) {
  ...
  replaceVars()
  stream = gulp.src([opts.config || config.variable, path.resolve(config.themePath, './src/*.css')])
    .pipe(postcss([salad(opts.browsers)]))
    .pipe(opts.minimize ? cssmin({showLog: false}) : nop())
    .pipe(gulp.dest(opts.out || config.output))
    .on('end', function () {
      spin.succeed()
    })
